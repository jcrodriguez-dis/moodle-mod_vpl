<?php
    ...
    final class seb_settings_test extends base_fixture {
        ...
        public function test_download_config_xml_contains_expected_entries(): void {
            ...
            $config = \mod_vpl\seb\settings::build_download_config_xml($record, new \moodle_url('/mod/vpl/view.php', ['id' => $instance->cmid]));
        ...
        /**
         * Ensure a denied SEB access attempt is logged when the config hash is invalid.
         */
        public function test_validate_access_triggers_event_when_config_hash_invalid(): void {
            global $USER;

            $instance = $this->getDataGenerator()->create_module('vpl', [
                'course' => $this->course->id,
                'name' => 'VPL with SEB access event',
                'requiresafeexambrowser' => 1,
            ]);
            ...
            foreach ($events as $event) {
                if ($event instanceof \mod_vpl\event\seb_access_denied) {
                    ...
                    $this->assertEquals('invalid_config_hash', $event->other['reason']);
                }
            }
