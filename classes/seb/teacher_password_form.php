...
/**
 * Form to request the teacher password to change SEB session.
 *
 * @package mod_vpl
...
/**
 * Class to define the teacher password form for VPL
 *
 * This form is used to request the teacher password before changing the SEB session.
 */
class teacher_password_form extends \moodleform {
    /**
    ...
    protected function definition() {
        ...
        global $SESSION;
        $mform = & $this->_form;
        $mform->addElement('header', 'headerpassword', get_string('sebsimultaneoussessionblocked', VPL));
        $parms = [
        ...
        $mform->addElement('passwordunmask', 'sebteacherpassword', get_string('password'));
        $mform->setType('sebteacherpassword', PARAM_TEXT);
        $mform->setDefault('sebteacherpassword', '');
    ...
    public static function get_attempt_var($vpl) {
        return 'vpl_sebteacherpassword_attempt' . $vpl->get_instance()->id;
    }
